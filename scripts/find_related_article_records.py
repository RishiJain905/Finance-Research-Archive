...
Provides:
- find_related_article_records(): Find article records related to a quant
"""
...
def compute_link_score(
    quant_record: dict, article_record: dict
    ...
    topic_score, shared_topics = compute_topic_overlap_score(
        quant_record, article_record
    )

    # Compatibility bonus (same function, arguments swapped)
    ...
    if compatibility_bonus > 0:
        reason_parts.append("provides narrative context")
...
def find_related_article_records(
    quant_record: dict,
    ...
    for article in article_records:
        score, shared_topics, reason = compute_link_score(quant_record, article)
